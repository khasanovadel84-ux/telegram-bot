...
def send_welcome(message):
    try:
        print(f"[UPDATE] /start от @{message.from_user.username} (id={message.chat.id})", flush=True)
        bot.reply_to(message, "Привет! Я твой первый ИИ-помощник")
...
def handle_text(message):
    try:
        print(f"[UPDATE] Текст от @{message.from_user.username} (id={message.chat.id}): {message.text}", flush=True)
